...
static t_result	parse_redirection(t_ast *cmd_node, t_token **current, t_allocs *allocs);
...
static t_result	parse_redirection(t_ast *cmd_node, t_token **current, t_allocs *allocs)
{
	t_token		*redirect_tok = *current;
	t_token		*file_tok;
	t_in_out	*io;

	if (!redirect_tok || !(redirect_tok->type == TOKEN_REDIR_IN || redirect_tok->type == TOKEN_HEREDOC
			|| redirect_tok->type == TOKEN_REDIR_OUT || redirect_tok->type == TOKEN_APPEND))
...
static t_result	parse_command(t_token **current, t_allocs *allocs)
{
	...
	cmd_node->data.cmd_node.io = (t_in_out *)arena_alloc(allocs->parse_alloc, sizeof(t_in_out));
	...
	{
		t_token_type type = peek_token_type(*current);
		if (type == TOKEN_WORD || type == TOKEN_SUBSTITUTION || type == TOKEN_WILDCARD)
		{
			char **new_argv = arena_alloc(allocs->parse_alloc, (cmd_node->data.cmd_node.argc + 2) * sizeof(char *));
			if (!new_argv)
				return (create_error(NO_MEMORY));
			ft_memcpy(new_argv, cmd_node->data.cmd_node.argv, cmd_node->data.cmd_node.argc * sizeof(char *));
		...
		{
			result = parse_redirection(cmd_node, current, allocs);
			if (result.is_error)
				return (result);
		}
		else
...
t_result	parse_cmdln(char *cmdln, t_mshell *shell, t_allocs *allocs)
{
	...
	t_result parse_result = parse_expression(&current, allocs);
