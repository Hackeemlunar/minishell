...
/*   Updated: 2025/05/14 22:25:04 by sngantch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	collect_pipeline_cmds(t_ast *node, t_ast **out_cmds, int max)
{
	...
	{
		left_count = collect_pipeline_cmds(node->data.bin_op_node.left,
				out_cmds, max);
		if (left_count < 0)
			return (-1);
		if (left_count >= max)
...
t_result	run_simple_cmd(t_ast *ast, t_mshell *shell, t_allocs *allocs)
{
	...
	if (is_builtin(ast->data.cmd_node.argv[0]))
	{
		...
		if (!shell || !shell->env_table)
            return (create_error(INVALID_ENV));
		exec_builtin(ast->data.cmd_node.argv, shell, shell->env_table,
				&shell->exit_status);
...
int	handle_pipeline(t_ast **cmds, int ncmds, t_mshell *shell, t_allocs *allocs,
		t_table *table)
{
	...
	for (int i = 0; i < ncmds; i++)
	{
		...
		if (pids[i] == 0)
		{
			...
			// Check if command is a builtin and execute
			if (is_builtin(cmds[i]->data.cmd_node.argv[0]))
			{
				if (shell && table)
				{
					exec_builtin(cmds[i]->data.cmd_node.argv, shell, table,
						&shell->exit_status);
					exit(EXIT_SUCCESS);
				}
			}
			execve(cmds[i]->data.cmd_node.argv[0], cmds[i]->data.cmd_node.argv,
					shell->env);
			perror("execve");
			exit(EXIT_FAILURE);
		}
...
void	walk_ast(t_ast *ast, t_mshell *shell, t_allocs *allocs, t_table *table)
{
	...
	case NODE_PIPELINE:
	{
		n = collect_pipeline_cmds(ast, cmds, 128);
		if (n < 0)
		{
