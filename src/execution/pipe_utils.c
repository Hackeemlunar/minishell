...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/05/15 14:19:58 by hmensah-          #+#    #+#             */
/*   Updated: 2025/05/15 14:24:29 by hmensah-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	collect_pipeline_left(t_ast *node, t_ast **out_cmds, int max)
{
	...
	left_count = collect_pipeline_cmds
		(node->data.bin_op_node.left, out_cmds, max);
	if (left_count < 0 || left_count >= max)
		return (-1);
	return (left_count);
}

static int	collect_pipeline_right(t_ast *node, t_ast **out_cmds,
	int left_count, int max)
{
	int	right_count;
